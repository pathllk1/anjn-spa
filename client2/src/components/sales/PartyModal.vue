...
<template>
  ...
  <div class="flex-1 overflow-y-auto p-3 grid gap-2 bg-gray-50">
    <div v-if="filtered.length === 0" class="flex flex-col items-center justify-center py-12 text-gray-300">
    ...
    <div v-for="party in filtered" :key="String(party._id || party.id)"
         ...
         class="border border-gray-200 p-3 rounded-xl hover:border-blue-400 hover:shadow-md cursor-pointer flex justify-between items-center transition-all bg-white group">
      <div class="min-w-0 flex-1">
        <div class="font-bold text-blue-900 text-sm group-hover:text-blue-700 truncate">{{ party.firm }}</div>
        <div class="flex items-center flex-wrap gap-1.5 mt-1">
          <span class="text-[10px] font-mono bg-gray-100 px-1.5 py-0.5 rounded border border-gray-200 text-gray-600">{{ party.gstin }}</span>
      ...
      <span class="shrink-0 text-[10px] font-bold bg-blue-50 text-blue-600 border border-blue-100 px-2.5 py-1 rounded-full opacity-0 group-hover:opacity-100 transition-all ml-3">
        SELECT →
      </span>
    </div>
  </div>
